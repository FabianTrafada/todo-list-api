import type { Request, Response } from "express"
import { eq, count } from "drizzle-orm"
import { db } from "@/db"
import { todos } from "@/db/schema"

export const createTodo = async (req: Request, res: Response) => {
    const { title, description } = req.body
    if (!title || !description) return res.status(400).json({ error: "The title and description is required" })
    if (!req.user || !req.user.id) {
        return res.status(401).json({ error: "Unauthorized, user ID is missing." });
    }
    try {
        const [todo] = await db.insert(todos).values({
            title,
            description,
            userId: req.user.id
        }).returning()
        
        res.status(201).json(todo)
    } catch (error) {
        res.status(500).json({ error: 'Internal server error' })
    }
}

export const updateTodo = async (req: Request, res: Response) => {
    const id = parseInt(req.params.id)
    const { title, description } = req.body
    if (!title || !description) return res.status(400).json({ error: "Title and Description is required!" })

    try {
        const [todo] = await db.select().from(todos).where(eq(todos.id, id))
        if (!todo) return res.status(404).json({ error: "ToDo not found." })
        if (todo.userId !== req.user?.id) return res.status(403).json({ message: "Forbidden" })

        const [newTodo] = await db.update(todos)
            .set({
                title,
                description
            })
            .where(eq(todos.id, id))
            .returning()

        const { userId, ...todoReturn } = newTodo
        res.status(200).json(todoReturn)
    } catch (error) {
        res.status(500).json({ error: "Internal server error." })
    }
}

export const deleteTodo = async (req: Request, res: Response) => {
    const id = parseInt(req.params.id)
    try {
        const [todo] = await db.select().from(todos).where(eq(todos.id, id))
        if (!todo) return res.status(404).json({ error: "ToDo not found." })
        if (todo.userId !== req.user?.id) return res.status(403).json({ message: "Forbidden" })

        const [todoDeleted] = await db.delete(todos).where(eq(todos.id, id)).returning()
        res.status(204).json(todoDeleted)
    } catch (error) {
        res.status(500).json({ error: "Internal server error." })
    }
}

export const getTodos = async (req: Request, res: Response) => {
    const page = parseInt(req.query.page as string) || 1
    const limit = parseInt(req.query.limit as string) || 10
    
    try {
        const todosList = await db.select({
            id: todos.id,
            title: todos.title,
            description: todos.description
        })
        .from(todos)
        .limit(limit)
        .offset((page - 1) * limit)

        if (!todosList || todosList.length === 0) {
            return res.status(404).json({ message: "ToDos not founded." })
        }

        const [totalResult] = await db.select({ count: count() }).from(todos)
        const totalTodos = totalResult.count

        res.status(200).json({
            todos: todosList,
            total: totalTodos,
            page,
            limit
        })
    } catch (error) {
        res.status(400).json({ error })
    }
}